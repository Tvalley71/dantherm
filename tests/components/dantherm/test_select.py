"""Test the Dantherm select platform."""

from unittest.mock import AsyncMock

from config.custom_components.dantherm.const import DOMAIN
from config.custom_components.dantherm.select import async_setup_entry

from homeassistant.core import HomeAssistant


async def test_select_setup_no_device(hass: HomeAssistant) -> None:
    """Test select setup when device is None."""
...
async def test_select_setup_missing_device_object(hass: HomeAssistant) -> None:
    """Test select setup when device object is missing."""
    mock_config_entry = AsyncMock()
    mock_config_entry.entry_id = "test_entry"
...
async def test_select_setup_success(hass: HomeAssistant) -> None:
    """Test successful select setup."""
    mock_config_entry = AsyncMock()
    mock_config_entry.entry_id = "test_entry"
...
async def test_select_setup_missing_entry(hass: HomeAssistant) -> None:
    """Test select setup when entry is missing from hass.data."""
    mock_config_entry = AsyncMock()
